...
export default function ExamPage() {
  ...
  const handleGenerate = () => {
    if (qCount >= 20 && tier === 'free') {
        showToast(`Generating ${qCount} questions requires a Pro or Max subscription.`);
        return;
    }
    showToast("Generating Exam... (Backend processing)", "success");
  };

  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto pt-8 md:pt-12 relative">
      {toast.show && (
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-sm p-6">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {['general', 'caie', 'edexcel', 'aqa', 'ib', 'sat', 'fbise'].map((b) => (
              <button key={b} onClick={() => setBoard(b)} className={`px-4 py-2 rounded-full text-sm font-bold border-2 transition ${board === b ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 hover:border-purple-300 hover:bg-purple-50 dark:hover:bg-slate-700'}`}>
                {b.toUpperCase()}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-sm p-5">
            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Difficulty</label>
            <select className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2.5 bg-gray-50 dark:bg-[#0F172A] font-medium outline-none dark:text-white">
        ...
        <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <button onClick={() => setQType('mcq')} className={`p-4 rounded-2xl border-2 text-left transition ${qType === 'mcq' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/30' : 'border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}>
            ...
            <button onClick={() => setQType('structured')} className={`p-4 rounded-2xl border-2 text-left transition ${qType === 'structured' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}>
            ...
            <button onClick={() => setQType('essay')} className={`p-4 rounded-2xl border-2 text-left transition ${qType === 'essay' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/30' : 'border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}>
        ...
        <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-sm p-5">
            ...
            <div className="flex gap-4 flex-wrap">
                {[5, 10, 20, 30].map((count) => (
                    <button key={count} onClick={() => setQCount(count)} className={`relative px-6 py-3 rounded-xl border-2 font-bold text-sm transition ${qCount === count ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700'}`}>
                        {count} Questions
        ...
        <button onClick={handleGenerate} className="w-full bg-[#10B981] hover:bg-[#059669] text-white rounded-full font-bold transition py-4 text-lg shadow-lg shadow-emerald-200 dark:shadow-none">Generate Exam</button>
      </div>
    </div>
  );
}

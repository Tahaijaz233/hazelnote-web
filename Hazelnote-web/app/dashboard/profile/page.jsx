...
export default function ProfilePage() {
  ...
  // Prevents the bar from showing full if 0 generations are made
  const progressPercentage = Math.min(100, Math.max(0, (generationsToday / 1) * 100));
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-[24px] shadow-sm p-6">
          ...
          <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-3 mb-2 overflow-hidden shadow-inner">
            <div className="bg-green-500 h-3 rounded-full transition-all" style={{ width: tier === 'free' ? `${progressPercentage}%` : '5%' }}></div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-[24px] shadow-sm p-6">
          ...
          <div className="flex items-center justify-between py-2">
            ...
            {/* Pure Tailwind Toggle - Small and flawless */}
            <button 
                onClick={() => setIsDarkMode(!isDarkMode)} 
                className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors focus:outline-none ${isDarkMode ? 'bg-[#10B981]' : 'bg-gray-300'}`}
            >

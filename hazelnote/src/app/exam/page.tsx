'use client';import { useEffect, useState } from 'react';import Link from 'next/link';import { useRouter } from 'next/navigation';import {LayoutDashboard, PlusCircle, ClipboardList, UserCircle, HelpCircle,Menu, X, Search, FileText, List, Clock, Award, Save, Sparkles, Bot,CheckCircle, XCircle, BookOpen, Target,} from 'lucide-react';import { onAuthStateChanged } from 'firebase/auth';import { doc, getDoc } from 'firebase/firestore';import { auth, db } from '@/lib/firebase';import { safeParseJSON, saveToStorage, getCurrentMonth } from '@/lib/utils';import { StudySet } from '@/types';interface MarkingResult {score: number;maxScore: number;strengths: string;improvements: string;studyTips: string;}export default function Exam() {const router = useRouter();const [user, setUser] = useState(null);const [sidebarOpen, setSidebarOpen] = useState(false);const [tier, setTier] = useState<'free' | 'pro'>('free');const [studyHistory, setStudyHistory] = useState<StudySet[]>([]);const [selectedBoard, setSelectedBoard] = useState('general');const [selectedStudySet, setSelectedStudySet] = useState<number | null>(null);const [difficulty, setDifficulty] = useState('medium');const [timeLimit, setTimeLimit] = useState('30');const [questionType, setQuestionType] = useState('mcq');const [questionCount, setQuestionCount] = useState(5);const [isGeneratingExam, setIsGeneratingExam] = useState(false);const [isMarkingExam, setIsMarkingExam] = useState(false);const [examInProgress, setExamInProgress] = useState(false);const [examCompleted, setExamCompleted] = useState(false);const [timeRemaining, setTimeRemaining] = useState(0);const [examQuestions, setExamQuestions] = useState<any[]>([]);const [userAnswers, setUserAnswers] = useState<Record<number, string>>({});const [score, setScore] = useState(0);const [examSaved, setExamSaved] = useState(false);const [markingResults, setMarkingResults] = useState<MarkingResult[]>([]);useEffect(() => {const unsubscribe = onAuthStateChanged(auth, async (u) => {setUser(u);if (u) {const snap = await getDoc(doc(db, 'profiles', u.uid));if (snap.exists()) {const p = snap.data();setTier(p.is_pro ? 'pro' : 'free');}}});setStudyHistory(safeParseJSON('hz_study_history', []));return () => unsubscribe();}, []);useEffect(() => {let interval: NodeJS.Timeout;if (examInProgress && timeRemaining > 0 && timeLimit !== '0') {interval = setInterval(() => {setTimeRemaining(prev => {if (prev <= 1) { submitExam(); return 0; }return prev - 1;});}, 1000);}return () => clearInterval(interval);}, [examInProgress, timeRemaining, timeLimit]);const formatTime = (s: number) => ${Math.floor(s / 60)}:${(s % 60).toString().padStart(2, '0')};const callGemini = async (systemPrompt: string, userText: string): Promise => {const res = await fetch('/api/gemini', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ systemPrompt, userText }),});const data = await res.json();if (data.error) throw new Error(data.error);if (!data.result) throw new Error('Empty response from AI. Please try again.');return data.result;};const parseJSONResponse = (raw: string) => {let cleanJson = raw.replace(/json/gi, '').replace(//g, '').trim();const jsonArrayStart = cleanJson.indexOf('[');const jsonArrayEnd = cleanJson.lastIndexOf(']');if (jsonArrayStart !== -1 && jsonArrayEnd !== -1) {cleanJson = cleanJson.substring(jsonArrayStart, jsonArrayEnd + 1);}return JSON.parse(cleanJson);};const parseJSONObjectResponse = (raw: string) => {let cleanJson = raw.replace(/json/gi, '').replace(//g, '').trim();const jsonObjectStart = cleanJson.indexOf('{');const jsonObjectEnd = cleanJson.lastIndexOf('}');if (jsonObjectStart !== -1 && jsonObjectEnd !== -1) {cleanJson = cleanJson.substring(jsonObjectStart, jsonObjectEnd + 1);}return JSON.parse(cleanJson);};const generateExam = async () => {if (!selectedStudySet) { alert('Please select a study set first!'); return; }const studySet = studyHistory.find(s => s.id === selectedStudySet);if (!studySet) { alert('Study set not found.'); return; }setIsGeneratingExam(true);
try {
  if (questionType === 'mcq') {
    // ── MCQ Generation ──────────────────────────────────────────────────
...
Return ONLY a valid JSON array of objects with this EXACT structure (do not include markdown block markers):[{"question": "Question text here?","options": ["Option A text", "Option B text", "Option C text", "Option D text"],"answer": "A"}]Study Material:${studySet.parts[2]}`;    const raw = await callGemini(prompt, '');
    ...
    // ── Structured Short-Answer Generation ─────────────────────────────
...
Each question should require a detailed written response of 3–5 sentences covering key concepts.Return ONLY a valid JSON array of objects with this EXACT structure (do not include markdown block markers):[{"question": "Question requiring detailed explanation","modelAnswer": "Comprehensive model answer covering all key points, 3-5 sentences"}]Study Material:${studySet.parts[2]}`;    const raw = await callGemini(prompt, '');
    const jsonQuestions = parseJSONResponse(raw);
...
Each question requires a comprehensive, well-structured essay response.Return ONLY a valid JSON array of objects with this EXACT structure (do not include markdown block markers):[{"question": "Essay question requiring extended analysis and argumentation","criteria": "Key points that must be addressed for full marks, separated by semicolons"}]Study Material:${studySet.parts[2]}`;    const raw = await callGemini(prompt, '');
    const jsonQuestions = parseJSONResponse(raw);
...
};const submitExam = async () => {setExamInProgress(false);if (questionType === 'mcq') {
  let correct = 0;
...
Question: ${q.question}${contextLine}Student Answer: ${studentAnswer}Return ONLY a valid JSON object with this EXACT structure (no markdown block markers):{"score": 7,"strengths": "what was correct, well-expressed, or showed good understanding","improvements": "specific gaps, errors, or missing concepts","studyTips": "concrete topics/concepts the student should review to improve"}`;    try {
  ...
  setScore(Math.round((totalScore / (totalMax || 1)) * examQuestions.length)); // approximate scaled score
  setMarkingResults(results);
  setIsMarkingExam(false);
  setExamCompleted(true);
}
};const startNewExam = () => {setExamInProgress(false); setExamCompleted(false);setExamQuestions([]); setUserAnswers({}); setScore(0); setExamSaved(false);setMarkingResults([]);};const saveExamToFolder = () => {if (tier !== 'pro') { alert('Saving exams is a Pro feature. Upgrade to unlock!'); return; }const folders = safeParseJSON<any[]>('hz_folders', []);
...
};const boards = [{ id: 'general', label: 'General' }, { id: 'caie', label: 'CAIE (O/A Level)' },{ id: 'edexcel', label: 'Edexcel' }, { id: 'aqa', label: 'AQA' },{ id: 'ib', label: 'IB' }, { id: 'sat', label: 'SAT / AP' }, { id: 'fbise', label: 'FBISE / Matric' },];const Sidebar = () => (<aside className={w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col h-full z-50 fixed md:sticky top-0 left-0 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}}>HazelNoteby free-ed<button onClick={() => setSidebarOpen(false)} className="md:hidden p-2 text-gray-500 hover:bg-gray-100 rounded-lg transition dark:text-gray-400 dark:hover:bg-gray-800">Workspace Dashboard Create Notes Take an Exam Professor Hazel{tier === 'free' && ⚡ Upgrade to Pro} Profile & Settings Support);return ({sidebarOpen && <div onClick={() => setSidebarOpen(false)} className="fixed inset-0 bg-gray-900/50 z-40 md:hidden backdrop-blur-sm" />}<button onClick={() => setSidebarOpen(true)} className="hidden md:flex fixed top-4 left-4 z-30 p-2 bg-gray-800/80 backdrop-blur border border-gray-700 text-gray-300 hover:text-white hover:bg-gray-700 rounded-lg transition items-center gap-2">Menu<button onClick={() => setSidebarOpen(true)} className="p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition">HazelNote    <div className="p-6 md:p-8 max-w-4xl mx-auto pt-8 md:pt-12">

...
export default function Exam() {
  ...
  const generateExam = async () => {
    ...
    try {
      if (questionType === 'mcq') {
        ...
        setExamQuestions(jsonQuestions.map((q: any) => ({ question: q.question, options: q.options, answer: q.answer.trim().toUpperCase(), type: 'mcq' })));
        ...
        setExamQuestions(jsonQuestions.map((q: any) => ({ question: q.question, modelAnswer: q.modelAnswer, type: 'structured' })));
      } else if (questionType === 'essay') {
        const count = Math.min(questionCount, tier === 'pro' ? questionCount : 1);
        const prompt = `Based on the following study material, create ${count} essay question(s) at ${difficulty} difficulty.\n\nReturn ONLY a valid JSON array:\n[\n  {\n    "question": "Essay question requiring extended analysis",\n    "criteria": "Key points separated by semicolons"\n  }\n]\n\nStudy Material:\n${materials}`;
        const raw = await callGemini(prompt, '', true);
        const jsonQuestions = parseJSONResponse(raw);
        if (!Array.isArray(jsonQuestions) || jsonQuestions.length === 0) throw new Error('Could not parse essay questions.');
        setExamQuestions(jsonQuestions.map((q: any) => ({ question: q.question, criteria: q.criteria, type: 'essay' })));
      }
  ...
  const submitExam = async () => {
    ...
    if (questionType === 'mcq') {
      ...
      for (const [i, q] of examQuestions.entries()) {
        ...
        const contextLine = q.modelAnswer ? `Model Answer: ${q.modelAnswer}` : q.criteria ? `Marking Criteria: ${q.criteria}` : '';
        const prompt = `You are a fair expert examiner. Mark this student answer.\nQuestion: ${q.question}\n${contextLine}\nStudent Answer: ${studentAnswer}\n\nReturn ONLY a valid JSON object:\n{\n  "score": 7,\n  "strengths": "...",\n  "improvements": "...",\n  "studyTips": "..."\n}`;
        try {
          const raw = await callGemini(prompt, '', true);
  ...
  const saveExamToFolder = () => {
    ...
    const totalScore = questionType === 'mcq' ? score : markingResults.reduce((a, r) => a + r.score, 0);
    ...
    const savedSet: StudySet = {
      ...
      flashcardCount: 0, quizCount: examQuestions.length,
      parts: [`Exam: ${sourceTitles}`, `Score: ${totalScore}/${totalMax} (${pct}%)`, `## Exam Results\n\n**Score:** ${totalScore}/${totalMax} (${pct}%)\n**Type:** ${questionType}\n**Difficulty:** ${difficulty}\n**Questions:** ${examQuestions.length}\n**Date:** ${new Date().toLocaleDateString()}\n\n---\n\n${quizText}`, '', ''],
      podcast: '', chatCount: 0, folderId: examFolder.id,
    };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-900">
      ...
      <Sidebar />
      <main className="flex-1 h-full overflow-y-auto relative bg-slate-900">
        ...
        <div className="p-6 md:p-8 max-w-4xl mx-auto pt-8 md:pt-12">
          <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-2">Take an Exam</h2>
          <p className="text-gray-400 mb-8">Generate a customised exam based on your notes.</p>

          {!examInProgress && !examCompleted && !isMarkingExam && (
            <div className="space-y-6">
              <div className="glass-card p-6 bg-gray-800/50 border-gray-700">
                <h3 className="font-bold text-white mb-4 flex items-center gap-2"><Search className="w-5 h-5 text-purple-500" /> Board / Curriculum <span className="text-xs font-normal text-gray-400 ml-1">(optional)</span></h3>
                <div className="flex flex-wrap gap-2">
                  {boards.map(b => (
                    <button key={b.id} onClick={() => setSelectedBoard(b.id)} className={`px-4 py-2 rounded-full text-sm font-bold border-2 transition ${selectedBoard === b.id ? 'border-green-500 bg-green-900/30 text-green-400' : 'border-gray-600 text-gray-300 hover:border-purple-500'}`}>{b.label}</button>
                  ))}
                </div>
              </div>

              <div className="glass-card p-6 bg-gray-800/50 border-gray-700">
                <h3 className="font-bold text-white mb-4 flex items-center gap-2"><FileText className="w-5 h-5 text-green-500" /> Select Notes</h3>
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                  {studyHistory.length === 0 ? (
                    ...
                    <label key={set.id} className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition ${selectedStudySets.includes(set.id) ? 'bg-green-900/30 border-green-500 text-green-400' : 'border-gray-700 hover:bg-gray-700/50'}`}>
                      ...
                      <div className="flex-1">
                        <div className="font-bold text-gray-200 text-sm">{set.title}</div>
                        <div className="text-xs text-gray-500">{new Date(set.date).toLocaleDateString()}</div>
                      </div>
                    </label>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="glass-card p-5 bg-gray-800/50 border-gray-700">
                  <label className="block text-sm font-bold text-gray-300 mb-2">Difficulty</label>
                  <select value={difficulty} onChange={e => setDifficulty(e.target.value)} className="w-full border border-gray-600 rounded-xl px-3 py-2.5 bg-gray-900 text-white font-medium focus:outline-none focus:border-green-500">
                ...
                <div className="glass-card p-5 bg-gray-800/50 border-gray-700">
                  <label className="block text-sm font-bold text-gray-300 mb-2">Time Limit</label>
                  <select value={timeLimit} onChange={e => setTimeLimit(e.target.value)} className="w-full border border-gray-600 rounded-xl px-3 py-2.5 bg-gray-900 text-white font-medium focus:outline-none focus:border-green-500">
              ...
              <div className="glass-card p-5 bg-gray-800/50 border-gray-700">
                <h3 className="font-bold text-white mb-3 flex items-center gap-2"><List className="w-5 h-5 text-blue-500" /> Question Type</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {[
                    ...
                    <button key={qt.id}
                      ...
                      className={`p-4 rounded-2xl border-2 text-left transition ${questionType === qt.id ? 'border-green-500 bg-green-900/30' : 'border-gray-600 hover:border-blue-500'} ${qt.pro && tier !== 'pro' ? 'opacity-60' : ''}`}>
                      <div className={`font-bold mb-1 flex items-center gap-1.5 ${questionType === qt.id ? 'text-green-400' : 'text-gray-300'}`}>
                        {qt.label}
              ...
              <div className="glass-card p-5 bg-gray-800/50 border-gray-700">
                ...
                <div className="flex gap-2 flex-wrap">
                  {getAvailableCounts().map(count => (
                    <button key={count} onClick={() => tier === 'pro' || count === 5 ? setQuestionCount(count) : alert('Pro feature')}
                      className={`px-5 py-2.5 rounded-xl border-2 font-bold text-sm transition ${questionCount === count ? 'border-green-500 bg-green-900/30 text-green-400' : 'border-gray-600 text-gray-400'} ${tier !== 'pro' && count > 5 ? 'opacity-60' : ''}`}>
                ...
                <p className="text-xs text-gray-500 mt-2">{tier === 'pro' ? 'Pro plan: full access enabled.' : 'Free plan: 5 questions. Upgrade to Pro for full access.'}</p>
              </div>
          ...
          {examInProgress && (
            <div>
              <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div>
                  <h3 className="text-2xl font-extrabold text-white">Exam in Progress</h3>
                  {timeLimit !== '0' && <div className="mt-1 flex items-center gap-1.5 text-orange-400 font-bold"><Clock className="w-4 h-4" /> {formatTime(timeRemaining)}</div>}
                </div>
                  ...
                  <div key={i} className="bg-gray-800 border border-gray-700 p-6 rounded-2xl">
                    ...
                    {q.type === 'mcq' && (
                      <div className="space-y-2">
                        {q.options.map((opt: string, j: number) => {
                          ...
                          return (
                            <label key={j} className="flex items-center gap-3 p-3 border border-gray-700 rounded-xl cursor-pointer hover:bg-gray-700/50 transition">
                    ...
                    {q.type === 'structured' && (
                      <div>
                        ...
                        <textarea value={userAnswers[i] || ''} onChange={e => setUserAnswers(prev => ({ ...prev, [i]: e.target.value }))} rows={5}
                          className="w-full border border-gray-600 rounded-xl px-4 py-3 bg-gray-900 text-white text-sm focus:outline-none focus:border-green-500 resize-none"
                          placeholder="Write your answer here..." />
                        <p className="text-xs text-gray-500 mt-1 text-right">{(userAnswers[i] || '').length} characters</p>
                      </div>
                    )}
                    {q.type === 'essay' && (
                      <div>
                        <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Your Essay Response</label>
                        <textarea value={userAnswers[i] || ''} onChange={e => setUserAnswers(prev => ({ ...prev, [i]: e.target.value }))} rows={12}
                          className="w-full border border-gray-600 rounded-xl px-4 py-3 bg-gray-900 text-white text-sm focus:outline-none focus:border-green-500 resize-none"
                          placeholder="Write your essay here..." />
                        <p className="text-xs text-gray-500 mt-1 text-right">{(userAnswers[i] || '').length} characters · ~{Math.ceil((userAnswers[i] || '').split(/\s+/).filter(Boolean).length)} words</p>
                      </div>
                    )}
                  </div>
                ))}
          ...
          {examCompleted && (
            <div className="space-y-6">
              <div className="glass-card p-8 text-center bg-gray-800/50 border-gray-700">
                <div className="w-20 h-20 bg-green-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Award className="w-10 h-10 text-green-400" />
                </div>
                {questionType === 'mcq' ? (
                  <>
                    <h3 className="text-3xl font-extrabold text-white mb-2">Score: {score}/{examQuestions.length}</h3>
                    <p className="text-gray-400 mb-6">{Math.round((score / examQuestions.length) * 100)}% correct</p>
                  </>
                ) : (
                  <>

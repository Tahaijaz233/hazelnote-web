...
const [stats, setStats] = useState({streak:0,notes:0,lastDate:null,monthlySets:{}});
...
audioChunksRef.current = [];
  ...
  // Attempt to use webkitSpeechRecognition for real-time preview
...
const podWordLimit = tier==='pro'?1500:225; // 1.5 mins limit for free users, 10 mins for Pro
...
===SPLIT===
Section 3: DETAILED NOTES in Markdown format. Use tables and bold text. Inline math $formula$, block 
...
setIsAddingContextLoading(true);
try {
const prompt = You are an expert tutor. Integrate the new source material into the existing notes seamlessly. Return ONLY in this EXACT format: TITLE: [New Updated Title (4-8 words max)] ===SPLIT=== [Updated Comprehensive Notes in Markdown format];
...
newParts[2] = cleanNotes;
  ...
  // Update Podcast text with new context
  ...
  // Clear old cached audio to regenerate
...
{cards.length===0&&No flashcards parsed.}
...
{userAnswers[i]===q.answer?'Correct!':Incorrect. Correct answer: ${q.answer}}
{q.explanation||'No explanation provided.'}

)}

))}
{questions.length===0&&No quiz generated.}
{tier==='pro' && (
...
Workspace
<button onClick={()=>{setCurrentView('dashboard');setSidebarOpen(false);window.history.pushState(null,'','/dashboard/');}} className={w-full text-left sidebar-item ${currentView==='dashboard'?'active':''}}> Dashboard
<button onClick={()=>{setCurrentView('create');setSidebarOpen(false);window.history.pushState(null,'','/dashboard/');}} className={w-full text-left sidebar-item ${currentView==='create'?'active':''}}> Create Notes
 Take an Exam
 Professor Hazel


{tier==='free'?(
<button onClick={()=>setGoProModalOpen(true)} className="w-full go-pro-badge py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 text-sm">⚡ Upgrade to Pro
):(

Pro Plan Active
<RefreshCw className={w-3 h-3 ${syncStatus==='syncing'?'animate-spin':''}}/>{syncStatus==='synced'?'Synced':syncStatus==='syncing'?'Syncing...':'Offline'}

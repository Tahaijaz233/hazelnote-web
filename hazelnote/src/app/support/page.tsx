...
export default function Support() {
  ...
  const Sidebar = () => (
    <aside className={`w-72 bg-gray-900 border-r border-gray-800 flex flex-col h-full z-50 fixed md:sticky top-0 left-0 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
      <div className="p-6 flex items-center justify-between">
        ...
        <button onClick={() => setSidebarOpen(false)} className="md:hidden p-2 text-gray-400 hover:bg-gray-800 rounded-lg transition">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="p-4 border-t border-gray-800 space-y-1">
        ...
        <Link href="/support/" className="w-full text-left sidebar-item active flex items-center gap-3 font-medium text-gray-400 hover:text-white"><HelpCircle className="w-5 h-5" /> Support</Link>
      </div>
    </aside>
  );
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-900">
      ...
      <Sidebar />
      <main className="flex-1 h-full overflow-y-auto bg-slate-900 relative">
        <button
        ...
        <div className="max-w-4xl mx-auto p-8 pt-8 md:pt-12">
          <div className="glass-card p-8 mb-6 bg-gray-800/50 border-gray-700">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-green-900/30 rounded-2xl flex items-center justify-center">
                <HelpCircle className="w-6 h-6 text-green-400" />
              </div>
              <div>
                <h1 className="text-3xl font-extrabold text-white">Support & Help</h1>
                <p className="text-gray-400">Get answers to your questions</p>
              </div>
            </div>
            ...
            <div className="bg-blue-900/20 border border-blue-800 rounded-2xl p-6 mb-8">
              ...
              <p className="text-blue-400 mb-3">Contact our support team and we&apos;ll get back to you within 24 hours.</p>
            ...
            <div className="space-y-4">
              {faqs.map((faq, i) => (
                <details key={i} className="group bg-gray-700/50 rounded-2xl overflow-hidden border border-gray-700">
                  <summary className="cursor-pointer p-5 font-bold text-white flex items-center justify-between hover:bg-gray-700 transition">
